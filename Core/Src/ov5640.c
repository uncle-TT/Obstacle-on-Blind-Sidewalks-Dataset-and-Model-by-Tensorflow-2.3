...
{
    SCCB_WR_Reg(0x3212,0x03);	//start group 3
    SCCB_WR_Reg(0x3a0f,OV5640_EXPOSURE_TBL[exposure][0]);
    SCCB_WR_Reg(0x3a10,OV5640_EXPOSURE_TBL[exposure][1]);
    SCCB_WR_Reg(0x3a1b,OV5640_EXPOSURE_TBL[exposure][2]);
    SCCB_WR_Reg(0x3a1e,OV5640_EXPOSURE_TBL[exposure][3]);
    SCCB_WR_Reg(0x3a11,OV5640_EXPOSURE_TBL[exposure][4]);
    SCCB_WR_Reg(0x3a1f,OV5640_EXPOSURE_TBL[exposure][5]);
    SCCB_WR_Reg(0x3212,0x13); //end group 3
    SCCB_WR_Reg(0x3212,0xa3); //launch group 3
}
...
void OV5640_Light_Mode(uint8_t mode)
{
    ...
    for(i=0; i<7; i++) SCCB_WR_Reg(0x3400+i,OV5640_LIGHTMODE_TBL[mode][i]);
...
void OV5640_Color_Saturation(uint8_t sat)
{
    uint8_t i;
    SCCB_WR_Reg(0x3212,0x03);	//start group 3
    SCCB_WR_Reg(0x5381,0x1c);
    SCCB_WR_Reg(0x5382,0x5a);
    SCCB_WR_Reg(0x5383,0x06);
    for(i=0; i<6; i++)SCCB_WR_Reg(0x5384+i,OV5640_SATURATION_TBL[sat][i]);
    SCCB_WR_Reg(0x538b, 0x98);
    SCCB_WR_Reg(0x538a, 0x01);
    SCCB_WR_Reg(0x3212, 0x13); //end group 3
    SCCB_WR_Reg(0x3212, 0xa3); //launch group 3
}
...
void OV5640_Brightness(uint8_t bright)
{
    ...
    SCCB_WR_Reg(0x3212,0x03);	//start group 3
    SCCB_WR_Reg(0x5587,brtval<<4);
    if(bright<4)SCCB_WR_Reg(0x5588,0x09);
    else SCCB_WR_Reg(0x5588,0x01);
    SCCB_WR_Reg(0x3212,0x13); //end group 3
    SCCB_WR_Reg(0x3212,0xa3); //launch group 3
}
...
void OV5640_Contrast(uint8_t contrast)
{
    ...
    SCCB_WR_Reg(0x3212,0x03); //start group 3
    SCCB_WR_Reg(0x5585,reg0val);
    SCCB_WR_Reg(0x5586,reg1val);
    SCCB_WR_Reg(0x3212,0x13); //end group 3
    SCCB_WR_Reg(0x3212,0xa3); //launch group 3
}

//sharp:0~33,0:close;33:auto
void OV5640_Sharpness(uint8_t sharp)
{
    if(sharp<33)//设置锐度
    {
        SCCB_WR_Reg(0x5308,0x65);
        SCCB_WR_Reg(0x5302,sharp);
    } else	//自动锐度
    {
        SCCB_WR_Reg(0x5308,0x25);
        SCCB_WR_Reg(0x5300,0x08);
        SCCB_WR_Reg(0x5301,0x30);
        SCCB_WR_Reg(0x5302,0x10);
        SCCB_WR_Reg(0x5303,0x00);
        SCCB_WR_Reg(0x5309,0x08);
        SCCB_WR_Reg(0x530a,0x30);
        SCCB_WR_Reg(0x530b,0x04);
        SCCB_WR_Reg(0x530c,0x06);
    }
...
const static uint8_t OV5640_EFFECTS_TBL[7][3]=
{
    ...
    0X1E,0xA0,0X40,//cold
    0X1E,0x80,0XC0,//warm
    0X1E,0x80,0X80,//black-white
    0X1E,0x40,0XA0,//yellow
    0X40,0x40,0X10,//anti
    0X1E,0x60,0X60,//green
};
void OV5640_Special_Effects(uint8_t eft)
{
    SCCB_WR_Reg(0x3212,0x03);
    SCCB_WR_Reg(0x5580,OV5640_EFFECTS_TBL[eft][0]);
    SCCB_WR_Reg(0x5583,OV5640_EFFECTS_TBL[eft][1]);
    SCCB_WR_Reg(0x5584,OV5640_EFFECTS_TBL[eft][2]);
    SCCB_WR_Reg(0x5003,0x08);
    SCCB_WR_Reg(0x3212,0x13);
    SCCB_WR_Reg(0x3212,0xa3);
}
...
uint8_t OV5640_ImageWin_Set(uint16_t offx,uint16_t offy,uint16_t width,uint16_t height)
{
    ...
    yend=offy+height-1;
    SCCB_WR_Reg(0X3212,0X03);
    SCCB_WR_Reg(0X3800,xst>>8);
    SCCB_WR_Reg(0X3801,xst&0XFF);
    SCCB_WR_Reg(0X3802,yst>>8);
    SCCB_WR_Reg(0X3803,yst&0XFF);
    SCCB_WR_Reg(0X3804,xend>>8);
    SCCB_WR_Reg(0X3805,xend&0XFF);
    SCCB_WR_Reg(0X3806,yend>>8);
    SCCB_WR_Reg(0X3807,yend&0XFF);
    SCCB_WR_Reg(0X3212,0X13);
    SCCB_WR_Reg(0X3212,0Xa3);
    return 0;
}
...
//Constant focus;return 0: ok
uint8_t OV5640_Focus_Constant(void)
{
//	unsigned char state = 0x8F;
    uint8_t temp=0;
    uint16_t retry=0;
    SCCB_WR_Reg(0x3023,0x01);
    SCCB_WR_Reg(0x3022,0x08);
    do
    {
        temp=SCCB_RD_Reg(0x3023);
        retry++;
        if(retry>1000)
            //user_main_error("OV5640_Constant_Focus time out !! %x\n",state);
            return 2;
        HAL_Delay(5);
    } while(temp!=0x00);
    SCCB_WR_Reg(0x3023,0x01);
    SCCB_WR_Reg(0x3022,0x04);
    retry=0;
    do
    {
        temp=SCCB_RD_Reg(0x3023);
    ...
    //user_main_printf("OV5640_Constant_Focus time out !!");
    return 0;
}
